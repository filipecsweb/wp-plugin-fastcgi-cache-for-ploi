 ...
 *     bloats the bundle.
 *  2. CSS scope. Every selector must start with the mount id, be a `tw:` utility,
 *     or only set custom properties, so the stylesheet can't restyle wp-admin.
  ...
  if (file.endsWith('.css')) {
    ...
    css.walkRules((rule) => {
      ...
      for (const selector of rule.selectors) {
        if (!selector.startsWith(ROOT) && !selector.includes('.tw\\:')) failures.push(`${file}: selector outside ${ROOT}: ${selector}`)
      }
    })
  }
}
